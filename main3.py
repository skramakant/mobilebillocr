...
def crop_based_on_template():
    cursor = sql_cursor_db()
    query = "select * from templatedata"
    cursor.execute(query)
    
    rows = cursor.fetchall()
    #list1 = list(rows[3][0])
    return json.loads(rows[3][0])
#    i=1
#    for row in rows:
#        if(i==4):
#            print(row)
...
def process_image_for_ocr():
    ...
    while True:
        ...
        if(key == ord("c")):
            selectedReagon = crop_based_on_template()
...
#            th3 = cv2.adaptiveThreshold(grayImage,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,125,2)
#            th4 = cv2.cvtColor(th3, cv2.COLOR_GRAY2BGR)
#            image = th4.copy()
            ...
            selectedReagon=[]
            image_pdf = Image(filename= inputDirPath+file, resolution=300)
            image_jpg = image_pdf.convert('jpg')
            newfile=preProcessDirPath+x[0]+".jpg"
            image_jpg.save(filename=newfile)
            image = cv2.imread(newfile)
#            grayImage = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
#            th3 = cv2.adaptiveThreshold(grayImage,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,97,2)
#            th4 = cv2.cvtColor(th3, cv2.COLOR_GRAY2BGR)
#            image = th4.copy()
            #imS = cv2.resize(image, (960, 540))
            #image = imS.copy()
...
# =============================================================================
    ...
    cursor = sql_cursor_db()
    if(clear_table_flag):
        trucate_table = "truncate table cropeddata"
        cursor.execute(trucate_table)
...
#    varlist.append(variable_2)
    ...
    var_string = json.dumps(data)
    query_string = "INSERT INTO templatedata (cropdata) VALUES (?)"
...
def click_and_crop(event, x, y, flags, param):
    ...
    # check to see if the left mouse button was released
        ...
        cv2.rectangle(image,refPt[0],(x,y),(0,255,0),4)
